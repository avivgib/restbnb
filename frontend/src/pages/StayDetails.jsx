...
import { loadStay, addStayMsg } from '../store/stay.actions'
import { format } from 'date-fns'

import { StayDetailsHeader } from '../cmps/stay details cmps/StayDetailsHeader'
import { StayGallery } from '../cmps/stay details cmps/StayGallery'
import { StaySummary } from '../cmps/stay details cmps/StaySummary'
import { StayHighlights } from '../cmps/stay details cmps/StayHighlights'
import { StayDescription } from '../cmps/stay details cmps/StayDescription'
import { StayAmenities } from '../cmps/stay details cmps/StayAmenities'
import { StayPolicies } from '../cmps/stay details cmps/StayPolicies'
import { StayHostInfo } from '../cmps/stay details cmps/StayHostInfo'
import { StayAvailability } from '../cmps/stay details cmps/StayAvailability'
import { StayRatingsBreakdown } from '../cmps/stay details cmps/StayRatingsBreakdown'
import { StayReviews } from '../cmps/stay details cmps/StayReviews'
import { StayMapLocation } from '../cmps/stay details cmps/StayMapLocation'
import { StayBookingCard } from '../cmps/stay details cmps/StayBookingCard'
import { AppModal } from '../cmps/stay details cmps/AppModal'
import { AppHeader } from '../cmps/AppHeader'
import { StayScrollNav } from '../cmps/stay details cmps/StayScrollNav'
import { StayDetailsTopHeader } from '../cmps/stay details cmps/StayDetailsTopHeader'
...
export function StayDetails() {
  ...
  useEffect(() => {
    ...
    const suggestedCheckIn = checkInFromURL
      ? new Date(checkInFromURL)
      : stay.availability?.suggestedDates?.checkIn
      ? new Date(stay.availability.suggestedDates.checkIn)
      : null

    const suggestedCheckOut = checkOutFromURL
      ? new Date(checkOutFromURL)
      : stay.availability?.suggestedDates?.checkOut
      ? new Date(stay.availability.suggestedDates.checkOut)
      : null
  ...
  useEffect(() => {
    const params = {}
    if (checkInDate) params.check_in = format(checkInDate, 'yyyy-MM-dd')
    if (checkOutDate) params.check_out = format(checkOutDate, 'yyyy-MM-dd')
  ...
  }, [checkInDate, checkOutDate, guests, setSearchParams])
  ...
  const reviewCount = stay.reviews?.length || 0
